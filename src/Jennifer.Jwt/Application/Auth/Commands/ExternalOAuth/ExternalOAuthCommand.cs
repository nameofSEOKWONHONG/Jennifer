...
public sealed record ExternalOAuthCommand(string Provider, string AccessToken):ICommand<IResult>;

public class ExternalOAuthHandler(
    UserManager<User> userManager,
    RoleManager<Role> roleManager,
    IExternalOAuthHandlerFactory externalOAuthHandlerFactory,
    IJwtService jwtService): ICommandHandler<ExternalOAuthCommand, IResult>
{
    public async Task<Result<IResult>> HandleAsync(ExternalOAuthCommand command, CancellationToken cancellationToken)
    {
        ...
        if (!verified.IsSuccess) return TypedResults.BadRequest(TokenResponse.Fail("Not found"));
